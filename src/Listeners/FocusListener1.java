	...
	public void focusLost(FocusEvent arg0) {
		...
		if (txt.getName().equals("txtbirthDate")) {
			if (txt.getText().trim().equals("") || txt.getText().trim().equals("Enter Birth Date...")) {
				...
				if (txt1.length != 3) {
					JOptionPane.showMessageDialog(addStudentdialog.getInstance(), "Format of date: DD.MM.YYYY");
					txt.setText("Enter Birth Date...");
					txt.setForeground(Color.RED);
					addStudentdialog.getInstance().setFalse(2);
					return;
				}
				addStudentdialog.getInstance().setBirthDate(LocalDate.of(Integer.parseInt(txt1[2]), Integer.parseInt(txt1[1]), Integer.parseInt(txt1[0])));
		...
		if (txt.getName().equals("txtAddress")) {
			if (txt.getText().trim().equals("") || txt.getText().trim().equals("Enter Address...")) {
				...
				if (txt1.length != 4) {
					JOptionPane.showMessageDialog(addStudentdialog.getInstance(), "Format of Address: Street Number City Country");
					txt.setText("Enter Address...");
					txt.setForeground(Color.RED);
					addStudentdialog.getInstance().setFalse(3);
					return;
